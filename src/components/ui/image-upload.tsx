...
export function ImageUpload({ images, onImagesChange, maxImages = 5, className = '' }: ImageUploadProps) {
  ...
  console.log('ImageUpload renderizado con:', {
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    console.log('Máximo permitido:', maxImages)

    setIsUploading(true)

    try {
      const newImages: string[] = []
      const availableSlots = maxImages - images.filter(img => img && img.trim() !== '').length
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        {images.filter(image => image && image.trim() !== '').length < maxImages && (
      ...
      {images.filter(image => image && image.trim() !== '').length === 0 && (
      ...
      {images.filter(image => image && image.trim() !== '').length > 0 && (
        <div className="text-sm text-gray-600">
          {images.filter(image => image && image.trim() !== '').length} de {maxImages} imágenes cargadas
        </div>
      )}
    </div>
  )
