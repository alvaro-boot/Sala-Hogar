...
export default function CategoryPage() {
  ...
  const { products, loadProducts, filteredProducts, filters, setFilters } = useProductStore()
  ...
  useEffect(() => {
    ...
    loadProducts()
    // Set the category filter based on the URL
    if (currentCategory) {
      setFilters({ category: currentCategory.id })
    }
  }, [loadProducts, setFilters, currentCategory])
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <h1 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-4">
            {currentCategory ? `Catálogo - ${currentCategory.name}` : "Catálogo de Productos"}
          </h1>
          <p className="text-lg text-gray-600">
            {currentCategory
              ? `Descubre nuestra selección de muebles para ${currentCategory.name.toLowerCase()}`
